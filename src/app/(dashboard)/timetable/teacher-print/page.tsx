...
export default async function TeacherTimetablePrintPage({
  ...
  searchParams: Promise<{ teacherId?: string; semester?: string; year?: string }>;
}) {
  const { teacherId, semester, year } = await searchParams;
  ...
  const { data: entries } = await (supabase as unknown as {
    ...
    .select("day_of_week, period, subject_code, subject_name, classroom, start_time, end_time")
    .eq("school_id", profile.school_id)
    .eq("teacher_user_id", teacherId)
    .eq("semester", sem);

  // Build grid: day(1-5) -> period(1-8) -> entry
  type Entry = { subjectCode: string | null; subjectName: string; classroom: string | null };
  ...
  const periods = DEFAULT_PERIODS.map((p) => ({
    ...p,
    start: entries?.find((e) => e.period === p.period)?.start_time ?? p.start,
    end: entries?.find((e) => e.period === p.period)?.end_time ?? p.end,
  }));
    ...
    <div className="bg-white min-h-screen font-[sans-serif]">
      ...
      <div className="p-6 print:p-4">
        <div className="text-center mb-4">
          ...
          <p className="font-bold">ตารางสอน</p>
          <p className="text-sm">ครูผู้สอน: {teacher?.full_name ?? "-"}</p>
                      ...
                      <td key={p.period} className="border border-gray-400 px-1 py-1 text-center align-middle min-w-[80px]">
                        {entry ? (
                          <div>
                            ...
                            {entry.classroom && <div className="text-[9px] text-gray-600">{entry.classroom}</div>}
                          </div>
                        ) : ""}
                      </td>
                    );
        ...
        <p className="mt-3 text-sm">รวมเวลาสอนทั้งหมด {totalHours} ชั่วโมง/สัปดาห์</p>
