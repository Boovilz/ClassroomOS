...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const structuredFindings = await getAiBehaviorAnalysis(studentId);
    const result = await getAiSynthesis({ studentName: student.full_name, domain: "behavior", structuredFindings });
    ...
    if (!result.notConfigured) {
      await saveAiGeneratedContent({
        ...
        contentType: "insight",
        domain: "behavior",
        title: `วิเคราะห์พฤติกรรม: ${student.full_name}`,
