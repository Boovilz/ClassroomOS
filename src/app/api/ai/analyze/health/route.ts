...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const structuredFindings = await getAiHealthAnalysis(studentId);
    const result = await getAiSynthesis({ studentName: student.full_name, domain: "health", structuredFindings });
    ...
    if (!result.notConfigured) {
      await saveAiGeneratedContent({
        ...
        contentType: "insight",
        domain: "health",
        title: `วิเคราะห์สุขภาพ: ${student.full_name}`,
